...
```
.venv\Scripts\python.exe run.py --user-name Doston             # the cloud stack (preset maya), auto language
...
| `local` | NVIDIA Parakeet TDT 0.6B int8 (sherpa-onnx) | Ollama `qwen3:4b-instruct` | Kokoro (ONNX, CPU) |

`maya` carries the `local` providers as **fallbacks**: when a cloud request cannot start or gives
no first result in time, that turn is served locally, the cloud provider is marked down for
...
**Brains** (`--brain`, `eva/config.py: BRAINS`): `qwen` (the eval winner: 6.0/10, passes every
honesty check), `gpt-oss` (`gpt-oss-120b`: bigger, 3.5/10 in the eval, leaks tool calls as JSON
text) and `local`. Both Cerebras brains send `reasoning_effort: "low"`.
...
`elevenlabs_key.txt` in the project root (or `CEREBRAS_API_KEY` / `ELEVENLABS_API_KEY`).

...
Builds a variant (brain x voice) through ``eva.session.build_session`` (so it composes
exactly what ``run.py`` runs: providers, language plan, persona, memory), the real Silero
segmenter, and either the real ``Player`` (``--speakers``) or a ``MockPlayer``
(``--silent``, the default: nothing plays, the bytes the player gets are what is
measured), then feeds the sample utterances as mic frames at controlled moments. Prints
a per-turn table and writes ``bench/out/e2e_<brain>+<voice>.json``.
...
from eva.config import BRAINS, DEFAULT_BRAIN, DEFAULT_VOICE, SAMPLES_DIR, VOICES, make_preset  # noqa: E402
...
async def run_real(args: argparse.Namespace) -> int:
    ...
    preset = make_preset(args.brain, args.voice)
    ...
    session = build_session(
        ...
        mute_fillers=args.mute_fillers, on_event=log,
    )
    ...
    if freed:
        console.print(f"freed the GPU: unloaded {', '.join(freed)}")
    t0 = time.perf_counter()
    await asyncio.gather(stt.warmup(), llm.warmup(), tts.warmup())
    warm_s = time.perf_counter() - t0
    gpu_mib = gpu_used_mib()
    console.print(f"warmup {warm_s:.2f}s, GPU {gpu_mib} MiB in use")
...
def main() -> int:
    ...
    ap.add_argument("--brain", default=DEFAULT_BRAIN, choices=list(BRAINS))
    ...
    ap.add_argument("--out", help="JSON output path (default bench/out/e2e_<brain>+<voice>.json)")
